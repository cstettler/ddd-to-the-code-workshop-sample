...
class UserRegistrationCompletedMessageListenerTests {
    ...
    void initializeWalletOnUserRegistrationCompletedEventDelivery() {
        ...
        // assert
        await().atMost(3, SECONDS).untilAsserted(() -> verify(this.walletService).initializeWallet(walletOwner("peter")));
    }
    ...
    void initializeWalletOnUserRegistrationCompletedEventRedelivery() {
        ...
        doThrow(walletAlreadyExists(walletOwner("peter"))).when(this.walletService).initializeWallet(walletOwner("peter"));
