...
class JdbcWalletRepositoryTests {
    ...
    void addAndGet_newWallet_storesWalletInDatabase() {
        ...
        Wallet wallet = wallet()
                .walletOwner("wallet-owner-1")
                .build();

        // act
        this.walletRepository.add(wallet);
        Wallet loadedWallet = this.walletRepository.get(wallet.walletOwner());

        // assert
        assertThat(rowCountInDatabase("wallet"), is(1));

        assertThat(loadedWallet, hasState(walletWith()
                .walletOwner(is(walletOwner("wallet-owner-1")))
        ));
    }

    @Test
    void add_walletWithSameWalletOwnerAlreadyExisting_throwsWalletAlreadyExistsException() {
        // arrange
    ...
    void get_nonExistingWallet_throwsWalletNotExistingException() {
        // arrange
        WalletOwner walletOwner = walletOwner("non-existing-wallet-owner");

        // act + assert
        assertThrows(WalletNotExistingException.class, () -> this.walletRepository.get(walletOwner));
    }
